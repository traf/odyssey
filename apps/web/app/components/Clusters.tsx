...
export default function Clusters({ clusters, selected, onSelect }: ClustersProps) {
  ...
  const label = selected === null
    ? "All clusters"
    : clusters.find((c) => c.id === selected)?.name ?? "All clusters";
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1.5 min-w-40 rounded-xl bg-background border border-white/10 p-1 shadow-xl z-50">
          <button
            className={`w-full text-left px-3 py-2 text-xs rounded-full cursor-pointer ${
